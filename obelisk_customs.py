...
try:
    ...
    print("❌ FATAL: steganography_module not available")
...
class ObeliskValidator:
    ...
    def __init__(self):
        self.steg = CardSteganography() if STEG_AVAILABLE else None
        ...
        try:
            ...
            validation_mark = {
                'obelisk_validation': {
                    ...
                    'status': 'PASSED',
                    'mark': 'GATE_1_CLEARED',
                    'access_granted': ['account_realm', 'rental_system', 'subscription_portal'],
            ...
            self.steg.embed_data(card_path, enhanced_data, overwrite=True)
...
class ObeliskMainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.status_label.setStyleSheet("font-size: 13px; color: #fca5a5; padding: 10px;")
    ...
    def create_scan_area(self):
        ...
        instructions.setStyleSheet("font-size: 16px; color: #fca5a5;")
        ...
        self.card_label.setStyleSheet("font-size: 12px; color: #7f1d1d; margin-top: 10px;")
    ...
    def validate_card(self):
        ...
        if is_valid:
            ...
            self.log("🗑️  Initiating AUTO-DELETE protocol...")
            
            self.status_label.setText("❌ FRAUDULENT CARD - Deleting...")
            QApplication.processEvents()
            
            # Delete the file (NO CONFIRMATION)
            try:
                Path(self.current_card_path).unlink()
                self.log(f"✅ File deleted: {Path(self.current_card_path).name}")
                self.log("\n🏛️ The Obelisk has spoken. Fraud eliminated.")
                ...
                self.status_label.setText("🏛️ Fraud eliminated. Submit another card.")
                ...
                QMessageBox.critical(
                    ...
                    f"Reason: {reason}\n\n"
                    f"💀 The card has been deleted.\n"
                    f"No confirmation. No mercy. No second chances.\n\n"
                    f"Only authentic Crimson Collective cards may pass."
                )
                
            except Exception as e:
                self.log(f"❌ Delete failed: {e}")
                self.status_label.setText(f"⚠️ Delete failed: {e}")
    
    def enter_account_realm(self):
        """Enter the Account Realm (Gate 2)"""
        ...
        # TODO: Launch Account Realm GUI here
        QMessageBox.information(
            self,
            "🚪 Account Realm",
            "🏛️ Gate 1 passed!\n\n"
            "🚧 Account Realm GUI not yet implemented.\n\n"
            "Coming soon:\n"
            "• Member account display\n"
            "• Book rental system\n"
            "• Subscription management\n"
            "• Payment portal\n\n"
            "The Obelisk has done its job. 🔮"
        )
