...
class PiperTTSThread(QThread):
    """Thread for Piper TTS with advanced audio processing"""
    finished = pyqtSignal()
    progress = pyqtSignal(int)
    error = pyqtSignal(str)
    
    def __init__(self, text, voice_model, settings):
        super().__init__()
        self.text = text
    ...
    def run(self):
        try:
            ...
            chunks = self.split_text_into_chunks(self.text, 300)
            total_chunks = len(chunks)
            ...
            for i, chunk in enumerate(chunks):
                ...
                if chunk.strip():
                    ...
                    audio_file = self.generate_piper_audio(chunk, i)
                    if audio_file:
                        # Apply audio effects
                        processed_file = self.apply_audio_effects(audio_file)
                        self.audio_files.append(processed_file)
                        # Play the processed audio
                        self.play_audio_file(processed_file)
    ...
    def generate_piper_audio(self, text, chunk_index):
        ...
        try:
            ...
            # Add speed control if available
            if 'speed' in self.settings:
                piper_cmd.extend(["--length_scale", str(self.settings['speed'])])
            ...
            if process.returncode != 0:
                self.error.emit(f"Piper error: {stderr}")
                return None
            
            return temp_wav.name
                
        except Exception as e:
            self.error.emit(f"Error generating Piper audio: {str(e)}")
            return None
    
    def apply_audio_effects(self, input_file):
        """Apply various audio effects to the generated speech"""
        try:
            # Load audio
            audio, sr = librosa.load(input_file, sr=None)
            
            # Apply pitch shifting
            if 'pitch' in self.settings and self.settings['pitch'] != 0:
                audio = librosa.effects.pitch_shift(audio, sr=sr, n_steps=self.settings['pitch'])
            
            # Apply formant shifting (simulate different voice characteristics)
            if 'formant' in self.settings and self.settings['formant'] != 0:
                ...
                formant_factor = 2 ** (self.settings['formant'] / 12.0)
                if formant_factor != 1.0:
                    magnitude = self.shift_formants(magnitude, formant_factor)
                
                audio = librosa.istft(magnitude * np.exp(1j * phase))
            
            # Apply reverb effect
            if 'reverb' in self.settings and self.settings['reverb'] > 0:
                audio = self.add_reverb(audio, sr, self.settings['reverb'])
            
            # Apply echo effect
            if 'echo' in self.settings and self.settings['echo'] > 0:
                audio = self.add_echo(audio, sr, self.settings['echo'])
            
            # Apply chorus effect
            if 'chorus' in self.settings and self.settings['chorus'] > 0:
                audio = self.add_chorus(audio, sr, self.settings['chorus'])
            
            # Apply distortion
            if 'distortion' in self.settings and self.settings['distortion'] > 0:
                audio = self.add_distortion(audio, self.settings['distortion'])
            
            # Apply noise gate
            if 'noise_gate' in self.settings and self.settings['noise_gate'] > 0:
                audio = self.apply_noise_gate(audio, self.settings['noise_gate'])
            
            # Apply EQ
            if 'eq_low' in self.settings or 'eq_mid' in self.settings or 'eq_high' in self.settings:
                audio = self.apply_eq(audio, sr, self.settings)
            
            # Apply compression
            if 'compression' in self.settings and self.settings['compression'] > 0:
                audio = self.apply_compression(audio, self.settings['compression'])
            
            # Apply volume
            if 'volume' in self.settings:
                audio = audio * (self.settings['volume'] / 100.0)
    ...
    def shift_formants(self, magnitude, factor):
        """Shift formants by scaling frequency bins"""
        freq_bins, time_bins = magnitude.shape
    ...
    def add_chorus(self, audio, sr, amount):
        ...
        for delay in delays:
            if delay < len(audio):
                delayed = np.zeros_like(audio)
                delayed[delay:] = audio[:-delay]
                chorus_audio += delayed * amount * 0.2
    ...
    def apply_noise_gate(self, audio, threshold):
        """Apply noise gate to reduce background noise"""
        threshold_linear = threshold / 100.0
        gate = np.where(np.abs(audio) > threshold_linear, 1.0, 0.1)
        return audio * gate
    ...
    def apply_eq(self, audio, sr, settings):
        ...
        # Get EQ gains (default to 0 if not specified)
        low_gain = settings.get('eq_low', 0) / 10.0  # Convert to linear scale
        mid_gain = settings.get('eq_mid', 0) / 10.0
        high_gain = settings.get('eq_high', 0) / 10.0
    ...
    def apply_compression(self, audio, amount):
        """Apply dynamic range compression"""
        threshold = 0.5
        ratio = 1 + amount * 0.1  # Compression ratio
...
class EdgeTTSApp(QMainWindow):
    ...
    def load_voices(self):
        ...
        try:
            ...
            # Create readable voice list
            self.available_voices = []
            for voice in voices:
                name = voice['ShortName']
                display_name = f"{voice['FriendlyName']} ({name})"
                self.available_voices.append((display_name, name))
                
            # Sort by language and gender
            self.available_voices.sort(key=lambda x: (x[1][:5], x[0]))
            
        except Exception as e:
            print(f"Error loading voices: {e}")
            # Fallback to common voices
            self.available_voices = [
                ("Aria (en-US-AriaNeural)", "en-US-AriaNeural"),
                ("Jenny (en-US-JennyNeural)", "en-US-JennyNeural"),
                ("Guy (en-US-GuyNeural)", "en-US-GuyNeural"),
                ("Davis (en-US-DavisNeural)", "en-US-DavisNeural"),
            ]
    ...
    def load_piper_models(self):
        ...
        try:
            ...
            if result.returncode != 0:
                raise Exception("Piper not found")
            
            # Common Piper models (user would need to download these)
            self.available_piper_models = [
                ("English (US) - Amy", "en_US-amy-medium"),
                ("English (US) - Danny", "en_US-danny-low"),
                ("English (US) - Joe", "en_US-joe-medium"),
                ("English (US) - Kathleen", "en_US-kathleen-low"),
                ("English (US) - Lessac", "en_US-lessac-medium"),
                ("English (UK) - Alan", "en_GB-alan-medium"),
                ("English (UK) - Alba", "en_GB-alba-medium"),
                ("French - Gilles", "fr_FR-gilles-low"),
                ("German - Thorsten", "de_DE-thorsten-medium"),
                ("Spanish - Davefx", "es_ES-davefx-medium"),
            ]
            
        except Exception as e:
            print(f"Piper not available: {e}")
            self.available_piper_models = [
                ("Demo Model (requires download)", "en_US-lessac-medium"),
            ]
    ...
    def init_ui(self):
        ...
        # Piper TTS Tab
        piper_tab = QWidget()
        self.setup_piper_tab(piper_tab)
        tab_widget.addTab(piper_tab, "🎭 Piper TTS")
    ...
    def setup_tts_tab(self, tab):
        ...
        """)
        for display_name, voice_name in self.available_voices:
            self.voice_combo.addItem(display_name, voice_name)
        # Set default to Aria
        for i, (_, voice_name) in enumerate(self.available_voices):
            if "AriaNeural" in voice_name:
    ...
    def setup_piper_tab(self, tab):
        """Setup Piper TTS tab with extensive voice modulation"""
        main_layout = QVBoxLayout()
        ...
        self.piper_text_edit = QTextEdit()
        self.piper_text_edit.setPlainText("Enter your text here for Piper TTS synthesis...")
        self.piper_text_edit.setFont(QFont("Arial", 12))
        self.piper_text_edit.setMaximumHeight(100)
        text_layout.addWidget(self.piper_text_edit)
        ...
        self.piper_model_combo.setObjectName("voiceCombo")  # Reuse the same styling
        self.piper_model_combo.setStyleSheet("""
            QComboBox#voiceCombo {
                background-color: #252525;
                border: 1px solid #CC1155;
                color: #FFFFFF;
        ...
        """)
        for display_name, model_name in self.available_piper_models:
            self.piper_model_combo.addItem(display_name, model_name)
        model_select_layout.addWidget(self.piper_model_combo)
        
        self.download_models_button = QPushButton("Download Models")
        self.download_models_button.setObjectName("downloadButton")  # Special styling
        self.download_models_button.setStyleSheet("""
        ...
        """)
        self.download_models_button.clicked.connect(self.download_piper_models)
        model_select_layout.addWidget(self.download_models_button)
        ...
        speed_layout = QHBoxLayout()
        speed_layout.addWidget(QLabel("Speed:"))
        self.piper_speed_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_speed_slider.setRange(50, 200)
        self.piper_speed_slider.setValue(100)
        self.piper_speed_slider.valueChanged.connect(self.update_piper_speed_label)
        speed_layout.addWidget(self.piper_speed_slider)
        self.piper_speed_label = QLabel("100%")
        speed_layout.addWidget(self.piper_speed_label)
        basic_layout.addLayout(speed_layout)
        
        # Volume control
        volume_layout = QHBoxLayout()
        volume_layout.addWidget(QLabel("Volume:"))
        self.piper_volume_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_volume_slider.setRange(0, 200)
        self.piper_volume_slider.setValue(100)
        self.piper_volume_slider.valueChanged.connect(self.update_piper_volume_label)
        volume_layout.addWidget(self.piper_volume_slider)
        self.piper_volume_label = QLabel("100%")
        volume_layout.addWidget(self.piper_volume_label)
        basic_layout.addLayout(volume_layout)
        ...
        pitch_layout = QHBoxLayout()
        pitch_layout.addWidget(QLabel("Pitch:"))
        self.piper_pitch_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_pitch_slider.setObjectName("pitchSlider")  # Special ID for custom styling
        self.piper_pitch_slider.setRange(-12, 12)
        self.piper_pitch_slider.setValue(0)
        self.piper_pitch_slider.valueChanged.connect(self.update_piper_pitch_label)
        pitch_layout.addWidget(self.piper_pitch_slider)
        self.piper_pitch_label = QLabel("0 semitones")
        self.piper_pitch_label.setProperty("accent", "true")
        pitch_layout.addWidget(self.piper_pitch_label)
        mod_layout.addLayout(pitch_layout)
        
        # Formant shifting
        formant_layout = QHBoxLayout()
        formant_layout.addWidget(QLabel("Formant:"))
        self.piper_formant_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_formant_slider.setRange(-12, 12)
        self.piper_formant_slider.setValue(0)
        self.piper_formant_slider.valueChanged.connect(self.update_piper_formant_label)
        formant_layout.addWidget(self.piper_formant_slider)
        self.piper_formant_label = QLabel("0 (neutral)")
        formant_layout.addWidget(self.piper_formant_label)
        mod_layout.addLayout(formant_layout)
        ...
        reverb_layout = QHBoxLayout()
        reverb_layout.addWidget(QLabel("Reverb:"))
        self.piper_reverb_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_reverb_slider.setRange(0, 100)
        self.piper_reverb_slider.setValue(0)
        self.piper_reverb_slider.valueChanged.connect(self.update_piper_reverb_label)
        reverb_layout.addWidget(self.piper_reverb_slider)
        self.piper_reverb_label = QLabel("0%")
        reverb_layout.addWidget(self.piper_reverb_label)
        effects_layout.addLayout(reverb_layout)
        
        # Echo
        echo_layout = QHBoxLayout()
        echo_layout.addWidget(QLabel("Echo:"))
        self.piper_echo_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_echo_slider.setRange(0, 100)
        self.piper_echo_slider.setValue(0)
        self.piper_echo_slider.valueChanged.connect(self.update_piper_echo_label)
        echo_layout.addWidget(self.piper_echo_slider)
        self.piper_echo_label = QLabel("0%")
        echo_layout.addWidget(self.piper_echo_label)
        effects_layout.addLayout(echo_layout)
        
        # Chorus
        chorus_layout = QHBoxLayout()
        chorus_layout.addWidget(QLabel("Chorus:"))
        self.piper_chorus_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_chorus_slider.setRange(0, 100)
        self.piper_chorus_slider.setValue(0)
        self.piper_chorus_slider.valueChanged.connect(self.update_piper_chorus_label)
        chorus_layout.addWidget(self.piper_chorus_slider)
        self.piper_chorus_label = QLabel("0%")
        chorus_layout.addWidget(self.piper_chorus_label)
        effects_layout.addLayout(chorus_layout)
        
        # Distortion
        distortion_layout = QHBoxLayout()
        distortion_layout.addWidget(QLabel("Distortion:"))
        self.piper_distortion_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_distortion_slider.setRange(0, 100)
        self.piper_distortion_slider.setValue(0)
        self.piper_distortion_slider.valueChanged.connect(self.update_piper_distortion_label)
        distortion_layout.addWidget(self.piper_distortion_slider)
        self.piper_distortion_label = QLabel("0%")
        distortion_layout.addWidget(self.piper_distortion_label)
        effects_layout.addLayout(distortion_layout)
        ...
        gate_layout = QHBoxLayout()
        gate_layout.addWidget(QLabel("Noise Gate:"))
        self.piper_gate_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_gate_slider.setRange(0, 100)
        self.piper_gate_slider.setValue(0)
        self.piper_gate_slider.valueChanged.connect(self.update_piper_gate_label)
        gate_layout.addWidget(self.piper_gate_slider)
        self.piper_gate_label = QLabel("0%")
        gate_layout.addWidget(self.piper_gate_label)
        processing_layout.addLayout(gate_layout)
        
        # Compression
        comp_layout = QHBoxLayout()
        comp_layout.addWidget(QLabel("Compression:"))
        self.piper_comp_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_comp_slider.setRange(0, 100)
        self.piper_comp_slider.setValue(0)
        self.piper_comp_slider.valueChanged.connect(self.update_piper_comp_label)
        comp_layout.addWidget(self.piper_comp_slider)
        self.piper_comp_label = QLabel("0%")
        comp_layout.addWidget(self.piper_comp_label)
        processing_layout.addLayout(comp_layout)
        ...
        eq_low_layout = QHBoxLayout()
        eq_low_layout.addWidget(QLabel("Low (< 300Hz):"))
        self.piper_eq_low_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_eq_low_slider.setRange(-20, 20)
        self.piper_eq_low_slider.setValue(0)
        self.piper_eq_low_slider.valueChanged.connect(self.update_piper_eq_low_label)
        eq_low_layout.addWidget(self.piper_eq_low_slider)
        self.piper_eq_low_label = QLabel("0 dB")
        eq_low_layout.addWidget(self.piper_eq_low_label)
        eq_layout.addLayout(eq_low_layout)
        
        # Mid EQ
        eq_mid_layout = QHBoxLayout()
        eq_mid_layout.addWidget(QLabel("Mid (300Hz-3kHz):"))
        self.piper_eq_mid_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_eq_mid_slider.setRange(-20, 20)
        self.piper_eq_mid_slider.setValue(0)
        self.piper_eq_mid_slider.valueChanged.connect(self.update_piper_eq_mid_label)
        eq_mid_layout.addWidget(self.piper_eq_mid_slider)
        self.piper_eq_mid_label = QLabel("0 dB")
        eq_mid_layout.addWidget(self.piper_eq_mid_label)
        eq_layout.addLayout(eq_mid_layout)
        
        # High EQ
        eq_high_layout = QHBoxLayout()
        eq_high_layout.addWidget(QLabel("High (> 3kHz):"))
        self.piper_eq_high_slider = QSlider(Qt.Orientation.Horizontal)
        self.piper_eq_high_slider.setRange(-20, 20)
        self.piper_eq_high_slider.setValue(0)
        self.piper_eq_high_slider.valueChanged.connect(self.update_piper_eq_high_label)
        eq_high_layout.addWidget(self.piper_eq_high_slider)
        self.piper_eq_high_label = QLabel("0 dB")
        eq_high_layout.addWidget(self.piper_eq_high_label)
        eq_layout.addLayout(eq_high_layout)
        ...
        # Text Formatting Options
        piper_format_group = QGroupBox("Text Formatting Options")
        piper_format_layout = QVBoxLayout()
        
        # Asterisk toggle
        piper_asterisk_layout = QHBoxLayout()
        self.piper_asterisk_toggle = QCheckBox("Read text inside *asterisks*")
        self.piper_asterisk_toggle.setChecked(True)  # Default to include asterisks content
        piper_asterisk_layout.addWidget(self.piper_asterisk_toggle)
        piper_format_layout.addLayout(piper_asterisk_layout)
        
        # Quotes toggle
        piper_quotes_layout = QHBoxLayout()
        self.piper_quotes_toggle = QCheckBox('Only read text inside "quotes"')
        self.piper_quotes_toggle.setChecked(False)  # Default to read all text
        piper_quotes_layout.addWidget(self.piper_quotes_toggle)
        piper_format_layout.addLayout(piper_quotes_layout)
        
        piper_format_group.setLayout(piper_format_layout)
        layout.addWidget(piper_format_group)
        ...
        self.reset_button.clicked.connect(self.reset_piper_settings)
        ...
        self.piper_play_button = QPushButton("▶ Generate & Play")
        self.piper_play_button.clicked.connect(self.play_piper_text)
        playback_layout.addWidget(self.piper_play_button)
        
        self.piper_stop_button = QPushButton("⏹ Stop")
        self.piper_stop_button.clicked.connect(self.stop_piper_speech)
        self.piper_stop_button.setEnabled(False)
        playback_layout.addWidget(self.piper_stop_button)
        
        self.piper_test_button = QPushButton("🔊 Test Settings")
        self.piper_test_button.clicked.connect(self.test_piper_voice)
        playback_layout.addWidget(self.piper_test_button)
        
        playback_group.setLayout(playback_layout)
        layout.addWidget(playback_group)
        
        # Progress bar
        self.piper_progress_bar = QProgressBar()
        self.piper_progress_bar.setObjectName("mainProgressBar")
        self.piper_progress_bar.setTextVisible(True)
        self.piper_progress_bar.setFormat("%p% Complete")
        layout.addWidget(self.piper_progress_bar)
    ...
    def setup_info_tab(self, tab):
        ...
        <ul>
            ...
            <li><b>Many Languages:</b> {len(set(voice[1][:5] for _, voice in self.available_voices))} languages available</li>
        ...
        <h3>Edge-TTS Voice Effects (NEW!):</h3>
        <ul>
        ...
        <h3>Piper TTS Features:</h3>
        <ul>
            <li><b>Open Source:</b> Fast, local neural text-to-speech</li>
            <li><b>High Quality:</b> Neural voices with natural prosody</li>
            <li><b>Extensive Modulation:</b> Pitch, formant, and effect controls</li>
            <li><b>Audio Effects:</b> Reverb, echo, chorus, distortion, EQ</li>
            <li><b>Voice Presets:</b> Pre-configured voice styles</li>
            <li><b>Real-time Processing:</b> Live audio effect application</li>
            <li><b>Completely Offline:</b> No internet required after setup</li>
        </ul>

        <h3>Available Voices: {len(self.available_voices)} Edge-TTS + {len(self.available_piper_models)} Piper</h3>
    ...
    # Piper TTS methods
    def update_piper_speed_label(self):
        self.piper_speed_label.setText(f"{self.piper_speed_slider.value()}%")
    
    def update_piper_volume_label(self):
        self.piper_volume_label.setText(f"{self.piper_volume_slider.value()}%")
    
    def update_piper_pitch_label(self):
        value = self.piper_pitch_slider.value()
        self.piper_pitch_label.setText(f"{value:+d} semitones")
    
    def update_piper_formant_label(self):
        value = self.piper_formant_slider.value()
        if value == 0:
            self.piper_formant_label.setText("0 (neutral)")
        elif value > 0:
            self.piper_formant_label.setText(f"+{value} (brighter)")
        else:
            self.piper_formant_label.setText(f"{value} (darker)")
    
    def update_piper_reverb_label(self):
        self.piper_reverb_label.setText(f"{self.piper_reverb_slider.value()}%")
    
    def update_piper_echo_label(self):
        self.piper_echo_label.setText(f"{self.piper_echo_slider.value()}%")
    
    def update_piper_chorus_label(self):
        self.piper_chorus_label.setText(f"{self.piper_chorus_slider.value()}%")
    
    def update_piper_distortion_label(self):
        self.piper_distortion_label.setText(f"{self.piper_distortion_slider.value()}%")
    
    def update_piper_gate_label(self):
        self.piper_gate_label.setText(f"{self.piper_gate_slider.value()}%")
    
    def update_piper_comp_label(self):
        self.piper_comp_label.setText(f"{self.piper_comp_slider.value()}%")
    
    def update_piper_eq_low_label(self):
        value = self.piper_eq_low_slider.value()
        self.piper_eq_low_label.setText(f"{value:+d} dB")
    
    def update_piper_eq_mid_label(self):
        value = self.piper_eq_mid_slider.value()
        self.piper_eq_mid_label.setText(f"{value:+d} dB")
    
    def update_piper_eq_high_label(self):
        value = self.piper_eq_high_slider.value()
        self.piper_eq_high_label.setText(f"{value:+d} dB")
    
    def download_piper_models(self):
        """Open instructions for downloading Piper models"""
        QMessageBox.information(
            self, "Download Piper Models", 
            "To download Piper models:\n\n"
            "1. Visit: https://github.com/rhasspy/piper/releases\n"
            "2. Download voice models (.onnx and .onnx.json files)\n"
            "3. Place them in your Piper models directory\n"
            "4. Restart the application\n\n"
            "Popular models:\n"
            "- en_US-lessac-medium (High quality English)\n"
            "- en_US-amy-medium (Female English)\n"
            "- en_US-joe-medium (Male English)"
        )
    
    def get_piper_settings(self):
        """Get current Piper settings as dictionary"""
        return {
            'speed': self.piper_speed_slider.value() / 100.0,
            'volume': self.piper_volume_slider.value(),
            'pitch': self.piper_pitch_slider.value(),
            'formant': self.piper_formant_slider.value(),
            'reverb': self.piper_reverb_slider.value() / 100.0,
            'echo': self.piper_echo_slider.value() / 100.0,
            'chorus': self.piper_chorus_slider.value() / 100.0,
            'distortion': self.piper_distortion_slider.value() / 100.0,
            'noise_gate': self.piper_gate_slider.value(),
            'compression': self.piper_comp_slider.value() / 100.0,
            'eq_low': self.piper_eq_low_slider.value(),
            'eq_mid': self.piper_eq_mid_slider.value(),
            'eq_high': self.piper_eq_high_slider.value(),
        }
    ...
    def load_preset(self, preset_name):
        ...
        presets = {
            "Default": {
                'speed': 100, 'volume': 100, 'pitch': 0, 'formant': 0,
                'reverb': 0, 'echo': 0, 'chorus': 0, 'distortion': 0,
                'noise_gate': 0, 'compression': 0, 
                'eq_low': 0, 'eq_mid': 0, 'eq_high': 0
            },
            "Radio Voice": {
                'speed': 95, 'volume': 110, 'pitch': -1, 'formant': 2,
                'reverb': 10, 'echo': 5, 'chorus': 0, 'distortion': 5,
                'noise_gate': 20, 'compression': 30,
                'eq_low': -3, 'eq_mid': 5, 'eq_high': 3
            },
            "Deep Voice": {
                'speed': 85, 'volume': 120, 'pitch': -8, 'formant': -5,
                'reverb': 15, 'echo': 0, 'chorus': 0, 'distortion': 0,
                'noise_gate': 10, 'compression': 20,
                'eq_low': 8, 'eq_mid': -2, 'eq_high': -5
            },
            "Chipmunk": {
                'speed': 150, 'volume': 90, 'pitch': 12, 'formant': 8,
                'reverb': 0, 'echo': 0, 'chorus': 20, 'distortion': 0,
                'noise_gate': 0, 'compression': 0,
                'eq_low': -10, 'eq_mid': 5, 'eq_high': 10
            },
            "Robot": {
                'speed': 80, 'volume': 100, 'pitch': -3, 'formant': 0,
                'reverb': 5, 'echo': 10, 'chorus': 0, 'distortion': 40,
                'noise_gate': 30, 'compression': 50,
                'eq_low': -5, 'eq_mid': 0, 'eq_high': 5
            },
            "Echo Chamber": {
                'speed': 90, 'volume': 100, 'pitch': 0, 'formant': 0,
                'reverb': 60, 'echo': 50, 'chorus': 10, 'distortion': 0,
                'noise_gate': 5, 'compression': 10,
                'eq_low': 2, 'eq_mid': -3, 'eq_high': 0
            },
            "Telephone": {
                'speed': 100, 'volume': 110, 'pitch': 0, 'formant': 0,
                'reverb': 0, 'echo': 0, 'chorus': 0, 'distortion': 10,
                'noise_gate': 40, 'compression': 60,
                'eq_low': -15, 'eq_mid': 8, 'eq_high': -10
            },
            "Stadium": {
                'speed': 95, 'volume': 130, 'pitch': 1, 'formant': 1,
                'reverb': 80, 'echo': 30, 'chorus': 5, 'distortion': 0,
                'noise_gate': 0, 'compression': 20,
                'eq_low': 3, 'eq_mid': 2, 'eq_high': 5
            },
            "Whisper": {
                'speed': 70, 'volume': 60, 'pitch': -2, 'formant': -1,
                'reverb': 20, 'echo': 0, 'chorus': 0, 'distortion': 0,
                'noise_gate': 60, 'compression': 80,
                'eq_low': -5, 'eq_mid': -8, 'eq_high': -3
            },
            "Dramatic": {
                'speed': 85, 'volume': 120, 'pitch': -2, 'formant': 1,
                'reverb': 40, 'echo': 15, 'chorus': 0, 'distortion': 0,
                'noise_gate': 10, 'compression': 40,
                'eq_low': 5, 'eq_mid': 3, 'eq_high': 2
            }
        }
        ...
        if preset_name in presets:
            settings = presets[preset_name]
            self.apply_piper_settings(settings)
    
    def apply_piper_settings(self, settings):
        """Apply settings to all controls"""
        self.piper_speed_slider.setValue(settings['speed'])
        self.piper_volume_slider.setValue(settings['volume'])
        self.piper_pitch_slider.setValue(settings['pitch'])
        self.piper_formant_slider.setValue(settings['formant'])
        self.piper_reverb_slider.setValue(int(settings['reverb']))
        self.piper_echo_slider.setValue(int(settings['echo']))
        self.piper_chorus_slider.setValue(int(settings['chorus']))
        self.piper_distortion_slider.setValue(int(settings['distortion']))
        self.piper_gate_slider.setValue(settings['noise_gate'])
        self.piper_comp_slider.setValue(int(settings['compression']))
        self.piper_eq_low_slider.setValue(settings['eq_low'])
        self.piper_eq_mid_slider.setValue(settings['eq_mid'])
        self.piper_eq_high_slider.setValue(settings['eq_high'])
    ...
    def reset_piper_settings(self):
        """Reset all Piper settings to default"""
        self.load_preset("Default")
    
    def test_piper_voice(self):
        """Test current Piper settings with a short phrase"""
        test_text = "Hello! This is a test of the Piper text-to-speech engine with custom voice effects and modulation."
        self.generate_piper_speech(test_text)
    
    def play_piper_text(self):
        """Start Piper text-to-speech playback"""
        text = self.piper_text_edit.toPlainText().strip()
        
        if not text or text == "Enter your text here for Piper TTS synthesis...":
            QMessageBox.warning(self, "Warning", "No text to speak!")
            return
            
        self.generate_piper_speech(text)
    
    def generate_piper_speech(self, text):
        """Generate speech using Piper TTS with effects"""
        if self.piper_thread and self.piper_thread.isRunning():
            return
        
        # Get selected voice model
        voice_model = self.piper_model_combo.currentData()
        if not voice_model:
            QMessageBox.warning(self, "Error", "Please select a voice model!")
            return
            
        # Process text based on formatting options
        text = self.process_text_formatting(text, 
                                           self.piper_asterisk_toggle.isChecked(),
                                           self.piper_quotes_toggle.isChecked())
        
        # Get current settings
        settings = self.get_piper_settings()
        
        # Start generation
        self.piper_thread = PiperTTSThread(text, voice_model, settings)
        self.piper_thread.finished.connect(self.on_piper_speech_finished)
        self.piper_thread.progress.connect(self.piper_progress_bar.setValue)
        self.piper_thread.error.connect(self.on_piper_speech_error)
        self.piper_thread.start()
        
        self.piper_play_button.setEnabled(False)
        self.piper_stop_button.setEnabled(True)
        self.piper_progress_bar.setValue(0)
    
    def stop_piper_speech(self):
        """Stop Piper text-to-speech playback"""
        if self.piper_thread:
            self.piper_thread.stop()
            self.piper_thread.wait()
        self.on_piper_speech_finished()
    
    def on_piper_speech_finished(self):
        """Handle Piper speech completion"""
        self.piper_play_button.setEnabled(True)
        self.piper_stop_button.setEnabled(False)
        self.piper_progress_bar.setValue(0)
    
    def on_piper_speech_error(self, error_msg):
        """Handle Piper speech errors"""
        QMessageBox.warning(self, "Piper TTS Error", f"Error during speech generation: {error_msg}")
        self.on_piper_speech_finished()
